...
- [x] **A11Y-01 — Correctifs accessibilité Lighthouse (`landmark-one-main` + `heading-order`)** — livré le 2026-09-25 sur branche `a11y-01-lighthouse-fixes` (2 commits `260a076`→`a6b8c57`, **locale, non poussée, non fusionnée, non déployée**), prolongement direct de TECH-10 (findings hors périmètre listés dans `docs/features/seo-geo/TECH-10-meta-viewport.md` ; audits à poids 3 expliquant le 96/100 de TECH-10). `app/[lang]/Home.tsx` : wrapper `<div className="w-full pt-16">` → `<main className="w-full pt-16 flex-1">` (pattern identique à `app/[lang]/cv/page.tsx:104`), `Footer` déplacé hors du landmark (frère du `<main>`) → un seul `<main>` sur `/fr`, `/en`, `/fr/cv`, `/en/cv`. `components/ExperienceGrid.tsx` : 3 titres de cartes `<h3>` → `<h2>` (classes Tailwind inchangées, rendu visuel identique) → hiérarchie `/fr` : `h1` ×1 → `h2` ×3, `h3` ×0. Revue glm-reviewer (contexte frais, 2026-09-25) : verdict **PROPRE**, aucune correction ; suggestions reportées avec motivation (JobMatcher `<h2>` rendu `ssr:false` hors DOM au chargement, pas de tests de composants dans le repo, `<header>` = `<nav>` pré-existant non signalé). Tests mesurés (2026-09-25, valides — aucune correction post-review) : `npm run lint` exit 0, `npm test` exit 0 — **3 fichiers, 71 tests passés** (102 ms), `npm run build` exit 0 (TS OK, 14 pages) ; vérif locale `next start` + curl `/fr` : `<main>` 1, `<h1>` 1, `<h2>` 3, `<h3>` 0, HTTP 200. **Validation opérateur : 2026-09-25.** État git : `main` porte aussi 1 commit docs local non poussé (`1f07020`, TECH-10) ; branche en attente de fusion/push.
