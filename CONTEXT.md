...
- ✅ `UX-004` (perte de focus de l'input chat sur envoi/Enter : input actif pendant le streaming + re-focus desktop dans `doSend()` + garde IME + `aria-busy` sur le conteneur de messages ; blur mobile `< 768 px` inchangé) — livré sur branche `ux-004-chat-input-focus` (1 commit `1b4040b`), **locale, non poussée ni fusionnée**, en attente de fusion/push ; revue PROPRE, 71/71 tests, validé opérateur 2026-09-25
