...
- ✅ `A11Y-01` (correctifs accessibilité Lighthouse : landmark `<main>` unique sur la home + `h3`→`h2` dans `ExperienceGrid` — prolongement direct de TECH-10) — livré sur branche `a11y-01-lighthouse-fixes` (2 commits), **locale, non poussée ni fusionnée**, en attente de fusion/push ; revue PROPRE, 71 tests passés
