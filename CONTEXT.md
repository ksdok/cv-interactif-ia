...
invariant éditorial M3 des projets, PROJ-001), soit **87 cas / 5 fichiers**.
...
- ✅ `OBS-001` (monitoring d'erreurs & alerting via Sentry `@sentry/nextjs`) — livré le 2026-09-25 sur branche `obs-001-sentry` (8 commits, `fadbc9b`→`3e0a237`), `@sentry/nextjs` épinglé exact 10.75.3, garde DSN, `tunnelRoute: '/monitoring'` (CSP inchangée), scrubbers vie privée, instrumentation `/api/chat`/`/api/job-match`/fallback provider, 78/78 tests, build secret-free prouvé ; **fusionné sur `main` et poussé sur `origin/main`** (`4d056a7`, merge commit, 2026-09-25) et **déployé en prod sur Vercel** le 2026-09-25 ; revue PROPRE, validé opérateur 2026-09-25 ; reste : création de l'alerting dans l'UI Sentry (action manuelle opérateur)
...
- ✅ `MODEL-003` (migration du SDK Gemini `@google/generative-ai` → `@google/genai@2.24.0` derrière le seam `callGemini*` : `callGemini` + `callGeminiStream`, chat session conservée, retry explicite `attempts: 2`, `systemInstruction` répété per-request en streaming, usage lu sur le dernier chunk ; ancien SDK retiré) — livré le 2026-09-25 sur branche `model-003-google-genai-migration` (3 commits `69ad957`→`6bb1cf7`, créée depuis `main` @ `781cc36`), **fusionnée sur `main` et poussée sur `origin/main`** (`882e3f9`, merge commit, 2026-09-25, déploiement à suivre) ; 78/78 tests, lint + type-check + build verts ; revue PROPRE, validé opérateur 2026-09-25 ; findings de revue corrigés (`GEMINI_USAGE_RE` suffixe `(stream)` + `PROVIDER_RE` `Streaming with provider:` + doc symétrique `docs/cag-limits.md`) ; cache Gemini toujours non confirmé
- ✅ `A11Y-01` (correctifs accessibilité Lighthouse : landmark `<main>` unique sur la home + `h3`→`h2` dans `ExperienceGrid` — prolongement direct de TECH-10) — livré sur branche `a11y-01-lighthouse-fixes` (2 commits), **fusionnée sur `main` et poussée sur `origin/main`** (`146770c`, merge commit, 2026-09-25 — déploiement à suivre) ; revue PROPRE, 71 tests passés
- ✅ `UX-004` (perte de focus de l'input chat sur envoi/Enter : input actif pendant le streaming + re-focus desktop dans `doSend()` + garde IME + `aria-busy` sur le conteneur de messages ; blur mobile `< 768 px` inchangé) — livré sur branche `ux-004-chat-input-focus` (1 commit `1b4040b`), **fusionné sur `main` et poussé sur `origin/main`** (`2a40e41`, merge commit, 2026-09-25 — déploiement à suivre) ; revue PROPRE, 71/71 tests, validé opérateur 2026-09-25
- ✅ `PROJ-001` (**page Projets GitHub bilingue** : hub SSR `/fr|/en/projets` + une page détail par projet, sans lien repo visible ; source éditoriale `content/projects.ts`, enrichissement GitHub server-only `lib/github.ts` (Data Cache 1 h + mémo négatif 5 min, résilience 401), JSON-LD noncé `ItemList`/`SoftwareSourceCode`, lien Projets header + footer, sitemap/llms.txt) - livré le 2026-09-26 sur branche `proj-001-projets-github` (9 commits `2185a06`→`4b9618d`, créée depuis `main` @ `e5a756a`), **NON fusionnée, NON poussée, NON déployée** ; 87/87 tests (5 fichiers), lint + type-check + build secret-free verts ; revue PROPRE (7 passages, cycle 1 corrigé) ; validé opérateur 2026-09-26 ; action opérateur hors code : `GITHUB_TOKEN` optionnel
